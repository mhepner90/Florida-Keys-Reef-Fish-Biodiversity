...
+ 1 - protected area
+ 0 - not protected 
- STRAT: A code indicating the stratum in which a sample was taken.   
+ FMLR
+ FSLR
+ HRRF
+ INPR
...
pdf("functional_dendrogram.pdf", width = 8, height = 30) 
par(pin=c(4,9))
#par(mar = c(3,3,3,7))
plot(dendrogram,
...
#Set rownames on traits and remove other info
  ...
  SAC_random = plot(Species_Accumulation_Curve, ci.type="poly", col="lightpink", lwd=2, ci.lty=0, ci.col="hotpink", xlim=range(1:100),xlab="Number of samples", ylab= "Number of species", main="Species Accumulation Curve", ylim=c(0,120))
  boxplot(Species_Accumulation_Curve, col="yellow", border="blue",add=TRUE, pch="+", xlim=range(1:100))
  ggsave(file="species_accum_curve.pdf",path="plots",SAC_random)
...
par(mfrow = c(1,2))
plot(Sp_rich, Srare, xlab = "Observed No. of Species", ylab = "Rarefied No. of Species", main="Rarefaction")
abline(0, 1)
#rarecurve(): draws a rarefaction curve for each row of the input data
...
ggsave(file="diversity_by_year_and_strata_grouped.pdf", g_gs, width=10, height=7, path="plots")

```

```{r plot each diversity by year and strata}
...
ggsave(file="richness_by_year_and_strata.pdf", path="plots")
...
shan_ys = ggplot(stats_all_matrices_data,aes(x=YEAR,y=shannon_mean, colour=STRAT, group=STRAT))+ #shape=STRAT
  ...
  theme(
      ...
      axis.title.y=element_text(size=12),
      title=element_text(size=12))
ggsave(file="shannon_by_year_and_strata.pdf", path="plots")
...
sim_ys = ggplot(stats_all_matrices_data,aes(x=YEAR,y=simpson_mean, colour=STRAT, group=STRAT))+ #shape=STRAT
  ...
  theme(
      ...
      axis.title.y=element_text(size=12),
      title=element_text(size=12))
ggsave(file="simpson_by_year_and_strata.pdf", path="plots")
...
func_ys= ggplot(stats_all_matrices_data,aes(x=YEAR,y=func.div_mean, colour=STRAT, group=STRAT))+ #shape=STRAT
  ...
  theme(
      ...
      axis.title.y=element_text(size=12),
      title=element_text(size=12))
ggsave(file="functional_by_year_and_strata.pdf", path="plots")

#save legend
for_legend_vert = ggplot(stats_all_matrices_data,aes(x=YEAR,y=richness_mean, colour=STRAT, group=STRAT))+
  geom_line(aes(group=STRAT),lwd=1) +
  geom_point(size=3) +
  labs(colour = "Strata")+
  theme(
...
ggsave(file="legend_year_and_strata.pdf", path="plots", width=8, height=1)

for_legend_horiz = ggplot(stats_all_matrices_data,aes(x=YEAR,y=richness_mean, colour=STRAT, group=STRAT))+
  geom_line(aes(group=STRAT),lwd=1) +
  geom_point(size=3) +
  labs(colour = "Strata")+
  theme(
...
g1 <- grid.arrange(arrangeGrob(rich_ys,shan_ys,ncol=2),arrangeGrob(sim_ys,func_ys,ncol=2),legend_horiz,nrow=3,heights=c(2,2,.4))
ggsave(file="diversity_by_year_and_strata.pdf", g1, width=10, height=8, path="plots")
...
diversity_strata = reshape2::melt(all_matrices_data, id.vars = c("STRAT"),
                                measure.vars=c("richness","shannon","simpson","func.div"))

diversity_strata=plyr::ddply(diversity_strata, c("STRAT", "variable"), summarize, 
                           value.mean=mean(value),
                           value.se=plotrix::std.error(value))
...
#   mutate(STRAT=
#            ifelse(grepl("FDLR", STRAT),'Forereef Deep Linear Reef',
#                   ifelse(grepl('FMLR', STRAT),'Foreef Medium Linear Reef',
#                          ifelse(grepl('FSLR', STRAT),'Foreef Shallow Linear Reef',
#                                 ifelse(grepl('HRRF', STRAT),'High Relief Reef',
#                                        ifelse(grepl('INPR', STRAT),'Inshore Patch Reef',
#                                               ifelse(grepl('MCPR', STRAT),'Midchannel Patch Reef',
#                                                      ifelse(grepl('OFPR', STRAT),'Offshore Patch Reef',"IDK"))))))))
                                       

diversity_strata_bargraph = diversity_strata %>%
  ggplot(.,aes(x=STRAT, y=value.mean,color=variable, group=variable, fill=variable))+ # 
  geom_errorbar(aes(ymax=value.mean+value.se, ymin=value.mean-value.se), size=.6,width=0.3,position=position_dodge(.9))+
  geom_bar(position="dodge",stat="identity", color="black",size=.3)+
...
DunnTest_grouped_strata = cbind(dt_richnnes_strat, dt_simpson_strat, dt_shannon_strat, dt_functional_strat)
