...
#plot functional dendrogram 
pdf("functional_dendrogram3.pdf", width = 9, height = 11) 
par(pin=c(5.5,9)) #current plot dimensions 
#par(mai = c(1,1,1,1))
plot(dendrogram,
     main= "", 
     xlab = "Dissimilarity",
...
dendrogram = dend %>%
color_branches(dend, k=10) %>% #color the branches based on the clusters 
  set("labels_cex", 0.8) %>% #size of the labels #0.8
  set("nodes_pch", 19) %>% #type of nodes
...
#plot functional dendrogram 
#pdf("functional_dendrogram4.pdf", width = 16, height = 20) 
tiff("functional_dendrogram4.tiff", width = 16, height = 20)
par(pin=c(12,16))
#par(mai = c(1,1,1,1))
plot(dendrogram,
     main= "", 
     xlab = "Figure 2. Functional Dissimilarity",
     cex.lab = 0.8, #The magnification to be used for x and y labels relative to the current setting of cex.
     cex.axis =0.8, #The magnification to be used for axis annotation relative to the current setting of cex.
     ylab = "",
     sub = NULL,
     horiz = T,
     ps=60, 
...
#for Ocean Sciences Poster 2018

diveristy_year_plot_enlarged = diversity_plot %>% 
    ggplot(.,aes(x=YEAR, y=value.mean, color=variable))+
    #geom_errorbar(aes(ymax=value.mean+value.se,ymin=value.mean-value.se),width=0.2)+
    geom_point(size=4)+
    geom_line(aes(group=variable),lwd=2.5)+
    labs(title= "", x="Year", y="Mean effective \nnumber of species", colour = "Diversity index")+
    scale_y_continuous(limits= c(0,45),
                       breaks=c(0,5,10,15,20,25,30,35,40,45), 
                       labels=c("0","","10","","20","","30","","40",""))+
    scale_x_continuous(limits = c(1999, 2016), breaks = c(1999,2000,2001,2002, 2003,2004,2005, 2006, 2007,2008, 2009,2010,2011, 2012,2013, 2014,2015, 2016), labels=c("","2000","","2002","","2004","","2006","","2008","","2010","","2012","","2014","","2016"))+
  scale_color_manual(
    labels= c("Richness","Shannon","Simpson","Functional"),
    values=c("Richness"="red",
             "Shannon"="orange",
  ...
  theme(
      ...
      legend.title =element_blank())
ggsave(file="diversity_by_year_enlarged.pdf", width=10, height=7, path="plots")
```
...
group_prot_OSM_poster <- grid.arrange(abun_prot,bio_prot,eve_prot,rich_prot,shan_prot,simp_prot,func_prot, legend_prot_vert, ncol=2)
ggsave(file="all_by_year_and_protection_poster.pdf", group_prot_OSM_poster, width=7.5, height=10, path="plots")
...
```{r plot all indices by year and dif marine zones}

all_matrices_data = read_csv("diversity_data.csv")

matrices_zones = all_matrices_data %>% 
    group_by(YEAR, MPA_NAME) %>% #add subregion later 
    summarize(
      abundance_mean = mean(abundance_sum),
      abundance_n = length(abundance_sum),
      abundance_sd = sd(abundance_sum),
...
#abundance 
abun_zones = ggplot(matrices_zones,aes(x=YEAR,y=abundance_mean, colour=factor(MPA_NAME), group=factor(MPA_NAME)))+ 
  geom_line(aes(group=MPA_NAME),lwd=1) +
  geom_point(size=3) +
  geom_errorbar(aes(ymax=abundance_mean+abundance_se,ymin=abundance_mean-abundance_se),width=0.1)+
  labs(title= "A) Abundance", x="Year", y="Mean abundance (± SE)", color = NULL)+
  scale_color_manual(
    values =  c("blue","red","orange","forestgreen"),
    breaks = c("Not Protected","Ecological Reserve","Sanctuary Preservation Area","Special Use"), 
...
group_prot_OSM_poster <- grid.arrange(abun_prot,bio_prot,eve_prot,rich_prot,shan_prot,simp_prot,func_prot, ncol=2)
ggsave(file="all_by_year_and_protection_poster.pdf", group_prot_OSM_poster, width=7.5, height=10, path="plots")
...
bio_gs= ggplot(matrices_strat_grouped,aes(x=YEAR,y=biomass_mean, colour=STRAT_GROUPED, group=STRAT_GROUPED))+ #shape=STRAT_GROUPED
 ...
 geom_errorbar(aes(ymin=biomass_mean-biomass_se, ymax=biomass_mean+biomass_se), width=.1)+
  labs(title= "B) Biomass", x="Year", y="Mean biomass \nkg per 40,000 km2 (± SE)", colour = "Strata")+
  scale_colour_manual(labels=c("High reef", "Linear reef", "Patch reef"), values= c("red", "green", "blue"))+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  theme_bw()+
  theme(
...
ggsave(file="all_by_year_and_strata_grouped_poster.pdf", g_gs, width=7, height=10, path="plots")
...
```{r bargraph by different ntmr}

diversity_ntmr = reshape2::melt(all_matrices_data, id.vars = c("MPA_NAME"),measure.vars=c("richness","shannon","simpson","func.div"))
#"abundance_sum","biomass_sum","evenness",

diversity_ntmr=plyr::ddply(diversity_ntmr, c("MPA_NAME", "variable"), summarize, 
                           value.mean=mean(value),
                           value.se=plotrix::std.error(value))
levels(diversity_ntmr$variable)=c("Richness", "Shannon", "Simpson", "Functional") #"Abundance","Biomass","Evenness",

x = c("Not Protected","Ecological Reserve","Sanctuary Preservation Area","Special Use")

as.tibble(diversity_ntmr)
diversity_ntmr = diversity_ntmr %>%
  mutate(MPA_NAME = factor(MPA_NAME, levels = x))%>%
  arrange(MPA_NAME)

diversity_ntmr_plot = diversity_ntmr %>%
  ggplot(.,aes(x=variable, y=value.mean, color=MPA_NAME, fill=MPA_NAME))+  
  geom_errorbar(aes(ymax=value.mean+value.se, ymin=value.mean-value.se), size=.6,width=0.3,position=position_dodge(.9))+
  geom_bar(position="dodge",stat="identity", color="black",size=.3)+
  labs(title="Biodiversity by Different Marine Zones", x="", y="",color="",fill="")+
   scale_fill_manual(
...
#Abundance on seperate plot 
diversity_ntmr_abun = reshape2::melt(all_matrices_data, id.vars = c("MPA_NAME"),measure.vars=c("abundance_sum"))
#"evenness",

diversity_ntmr_abun=plyr::ddply(diversity_ntmr_abun, c("MPA_NAME", "variable"), summarize, 
                           value.mean=mean(value),
                           value.se=plotrix::std.error(value))
levels(diversity_ntmr_abun$variable)=c("abundance_sum"="Abundance") 

x = c("Not Protected","Ecological Reserve","Sanctuary Preservation Area","Special Use")

as.tibble(diversity_ntmr_abun)
diversity_ntmr_abun = diversity_ntmr_abun %>%
  mutate(MPA_NAME = factor(MPA_NAME, levels = x))%>%
  arrange(MPA_NAME)

diversity_ntmr_abun_plot = diversity_ntmr_abun %>%
  ggplot(.,aes(x=variable, y=value.mean, color=MPA_NAME, fill=MPA_NAME))+  
  geom_errorbar(aes(ymax=value.mean+value.se, ymin=value.mean-value.se), size=.6,width=0.3,position=position_dodge(.9))+
  geom_bar(position="dodge",stat="identity", color="black",size=.3)+
...
ggsave(file="abundance_by_level_of_protection_bargraph.pdf",diversity_ntmr_abun_plot, width=10,height=4, path="plots")
#Biomass on seperate plot 

diversity_ntmr_bio = reshape2::melt(all_matrices_data, id.vars = c("MPA_NAME"),measure.vars=c("biomass_sum"))
#"evenness",

diversity_ntmr_bio=plyr::ddply(diversity_ntmr_bio, c("MPA_NAME", "variable"), summarize, 
                           value.mean=mean(value),
                           value.se=plotrix::std.error(value))
levels(diversity_ntmr_bio$variable)=c("biomass_sum"="Biomass") 

x = c("Not Protected","Ecological Reserve","Sanctuary Preservation Area","Special Use")

as.tibble(diversity_ntmr_bio)
diversity_ntmr_bio = diversity_ntmr_bio %>%
  mutate(MPA_NAME = factor(MPA_NAME, levels = x))%>%
  arrange(MPA_NAME)

diversity_ntmr_bio_plot = diversity_ntmr_bio %>%
  ggplot(.,aes(x=variable, y=value.mean, color=MPA_NAME, fill=MPA_NAME))+  
  geom_errorbar(aes(ymax=value.mean+value.se, ymin=value.mean-value.se), size=.6,width=0.3,position=position_dodge(.9))+
  geom_bar(position="dodge",stat="identity", color="black",size=.3)+
  labs(title="B) Biomass", x="", y="Mean Biomass \nkilograms per (± SE)",color="",fill="")+
   scale_fill_manual(
    labels= c("Not Protected","Ecological Reserve","Sanctuary Preservation Area","Special Use"),
     values=c(
              "Not Protected"="red",
...
abundance_ntmr_plot =  
  ...
  labs(title="A) Abundance", x="", y="Mean abundance (± SE)",color="",fill="")+ #\nnumber of individuals
  scale_fill_manual(
    labels= c("Not Protected","Ecological Reserve","Sanctuary Preservation Area","Special Use"),
    values=c(
      "Not Protected"="red",
...
biomass_ntmr_plot =  
  ...
  geom_bar(position="dodge",stat="identity")+ # color="black",size=.3
  labs(title="B) Biomass", x="", y="Mean biomass (± SE)",color="",fill="")+ #\nkilograms per 40,000 km2
  scale_fill_manual(
    labels= c("Not Protected","Ecological Reserve","Sanctuary Preservation Area","Special Use"),
    values=c(
      "Not Protected"="red",
...
richness_ntmr_plot =  
  ...
  geom_bar(position="dodge",stat="identity")+ # color="black",size=.3
  labs(title="D) Richness ", x="", y="Mean richness (± SE)",color="",fill="")+ #\neffective number of species
  scale_fill_manual(
    labels= c("Not Protected","Ecological Reserve","Sanctuary Preservation Area","Special Use"),
    values=c(
      "Not Protected"="red",
...
shannon_ntmr_plot =  
  ...
  geom_bar(position="dodge",stat="identity")+ # color="black",size=.3
  labs(title="E) Shannon", x="", y="Mean Shannon (± SE)",color="",fill="")+#\neffective number of species
  scale_fill_manual(
    labels= c("Not Protected","Ecological Reserve","Sanctuary Preservation Area","Special Use"),
    values=c(
      "Not Protected"="red",
...
simpson_ntmr_plot =  
  ...
  geom_bar(position="dodge",stat="identity")+ # color="black",size=.3
  labs(title="F) Simpson", x="", y="Mean Simpson (± SE)",color="",fill="")+ #\neffective number of species
  scale_fill_manual(
    labels= c("Not Protected","Ecological Reserve","Sanctuary Preservation Area","Special Use"),
    values=c(
      "Not Protected"="red",
...
functional_ntmr_plot =  
  ...
  geom_bar(position="dodge",stat="identity")+ # color="black",size=.3
  labs(title="G) Functional", x="", y="Mean functional (± SE)",color="",fill="")+ #\neffective number of species
  scale_fill_manual(
    labels= c("Not Protected","Ecological Reserve","Sanctuary Preservation Area","Special Use"),
    values=c(
      "Not Protected"="red",
...
ntmr_legend = get_legend(legend)

group_ntmr <- grid.arrange(abundance_ntmr_plot,biomass_ntmr_plot,evenness_ntmr_plot, richness_ntmr_plot,shannon_ntmr_plot,simpson_ntmr_plot,functional_ntmr_plot, ntmr_legend, ncol=2)

ggsave(file="all_by_ntmr_bargraph_noxaxis.pdf", group_ntmr, width=7.5, height=10, path="plots")
...
#Abundance 
kruskal.test(all_matrices_data$abundance_sum~all_matrices_data$MPA_NAME)
#Kruskal-Wallis chi-squared = 132.68, df = 3, p-value < 2.2e-16
dt_abun_mpa = dunnTest(abundance_sum~MPA_NAME, data=all_matrices_data)
dt_abundance_mpa = dt_abun_mpa$res
cldList(P.adj~Comparison,
        data = dt_abundance_mpa,
        threshold = 0.05)
#ER = Not Protected
#SPA = SU

#Biomass
kruskal.test(all_matrices_data$biomass_sum~all_matrices_data$MPA_NAME)
#Kruskal-Wallis chi-squared = 215.34, df = 3, p-value < 2.2e-16
dt_bio_mpa = dunnTest(biomass_sum~MPA_NAME, data=all_matrices_data)
dt_biomass_mpa = dt_abun_mpa$res
...
#Evenness
kruskal.test(all_matrices_data$evenness~all_matrices_data$MPA_NAME)
#Kruskal-Wallis chi-squared = 5.2612, df = 3, p-value <  0.1536
...
DunnTest_grouped_mpa = cbind(dt_abundance_mpa, dt_biomass_mpa, dt_richnnes_mpa, dt_shannon_mpa, dt_simpson_mpa, dt_functional_mpa)
write_csv(DunnTest_grouped_mpa, "DunnTest_grouped_mpa.csv")

## All indices are significantly different by level of protection except evenness
...
matrices_ntmr = all_matrices_data %>% 
    ...
    summarize(
      ...
      #func.div_max = max(func.div) %>%
  gather("INDICES", "MEAN", 2:8) %>%
  spread(MPA_NAME,MEAN) %>%
  mutate(DIFF= (`Ecological Reserve`-`Not Protected`),
         PERC= (1-(`Not Protected`/`Ecological Reserve`))*100)
...
ggsave(file="partial_deviance_explained.pdf", partial_deviance_explained, width=8, height=5, path="plots")
