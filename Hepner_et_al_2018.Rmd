...
```{r Dunn's test by strat grouped and protection}
...
```{r protection areas offset}

all_matrices_data = read_csv("diversity_data.csv")

offsets = all_matrices_data %>%
  select(YEAR, PRIMARY_SAMPLE_UNIT, PROT, richness) %>% 
  group_by(YEAR, PROT) %>%
  summarize(
      richness_mean = mean(richness)) %>%
  spread(PROT,richness_mean) %>%
  mutate(rich_offset = `1`-`0`)

rich_offset = offsets %>%
  ungroup() %>% 
  summarize(
      richness_offset = mean(rich_offset),
      richness_n_offset = length(rich_offset),
      richness_sd_offset = sd(rich_offset),
      richness_se_offset = richness_sd_offset / sqrt(richness_n_offset))
...
write_csv(prot_offset, "prot_offset.csv")
