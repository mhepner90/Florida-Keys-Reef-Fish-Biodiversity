...
trait_matrix_common = trait_matrix %>%
  ...
  names(all.trees)=c(tree_methods,"consensus")
  (trees.dissim=lapply(all.trees,function(i) cl_dissimilarity(i,traits.dist.common,method="spectral"))) 
...
diversity_indices = diversity_indices %>%
  ...
  mutate(simpson = round(simpson,6),
           shannon = round(shannon, 6))

write_csv(diversity_indices, "diversity_indices.csv")
...
abundance = clean_data %>%
  ...
  group_by(YEAR, PRIMARY_SAMPLE_UNIT,STRAT,PROT, SPECIES_CD)
...
#Drop communities with no species (roughly 500 sites)
  ...
  list = cbind(simpson,func.div,evenness) %>%
    as_tibble()%>%
    mutate(simpson = round(simpson,6),
           func.div = round(func.div, 6),
           evenness = round(evenness, 6))
               
  diversity = full_join(diversity_indices, list, by="simpson") %>%
    select(YEAR, PRIMARY_SAMPLE_UNIT, STRAT, PROT,
           richness,shannon,simpson,func.div,evenness)%>%
    group_by(YEAR, PRIMARY_SAMPLE_UNIT, STRAT,PROT) #5,235 groups
  
  write.csv(diversity, "diversity.csv")  #5247x9
  
  sample_data = read_csv("sample_data_all_years.csv") #5241x22
  
  RVC_data = read_rds("RVCdata_FK.rds")
  survey_data = RVC_data$sample_data %>%
    select(-SPECIES_CD, -SPECIES_NR, -REGION, -TIME_SEEN, -LEN, -NUM)%>%
    group_by(YEAR,PRIMARY_SAMPLE_UNIT)%>% #MPA_NR, PROT,MONTH (5241),
    summarize(
      LAT_DEGREES = mean(LAT_DEGREES),
      LON_DEGREES = mean(LON_DEGREES),
      DEPTH = mean(DEPTH)) #dim 5,245X9
